<div class='product-list-container mx-5 w-80'>
    ...
    $resultSet = $product->queryData("SELECT DISTINCT pd.product_id AS product_id, (SELECT c.img1 from product_colors c WHERE c.product_id=pd.product_id LIMIT 1) AS image, (SELECT  p.avg_star from products p where pd.product_id=p.product_id) AS avg_star,(SELECT  p.total_review from products p where pd.product_id=p.product_id) AS total_review, (SELECT  p.price from products p where pd.product_id=p.product_id) AS price  FROM products pd");
    ...
    <picture class='product-img my-5'>
        ...
        <ul class='product-variation d-flex justify-content-center'>
            ...
            <li class='product-variation-list'>
                <span style='background-size:100% 100%;background:green' class='product-variation-list-item '></span>
            </li>
            <li class='product-variation-list'>
                <span style='background-size:100% 100%;background:black' class='product-variation-list-item '></span>
            </li>
            <li class='product-variation-list'>
                <span style='background-size:100% 100%;background:linear-gradient(#000,#9a7a3f)' class='product-variation-list-item '></span>
            </li>
        ...
        <div class='product-info d-flex align-items-center justify-content-between'>
            <div class='d-flex align-items-center'>
                ...
                <?php echo "<p>{$result['product_id']}</p>
        </div>
        <span class=''> 
